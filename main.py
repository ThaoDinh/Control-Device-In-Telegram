...
from config.apiKey import ACCESS_TOKEN
...
def SendMessage(message):
    ...
    markup.add(types.InlineKeyboardButton(
        "Screenshot 🖥", callback_data="screenshot"))
    markup.add(types.InlineKeyboardButton(
        "Tasklist 📱", callback_data="tasklist"))
    markup.add(types.InlineKeyboardButton(
        "Show Text 📦", callback_data="showinbox"))
    markup.add(types.InlineKeyboardButton(
        "Open Url 〽️", callback_data="url"))
    markup.add(types.InlineKeyboardButton(
        "End Application 👏", callback_data="killapp"))
    markup.add(types.InlineKeyboardButton(
...
def callback(call):
    if call.data == "screenshot":
        ...
        bot.send_photo(call.message.chat.id, image_stream,
                       caption="Here's Screenshot !")
    elif call.data == "tasklist":
        running_apps = {proc.info['name']
...
@bot.message_handler(
    func=lambda message: waiting_for_url.get(message.chat.id, False))
def open_url(message):
    try:
        url = message.text
        webbrowser.open(url)
        bot.send_message(message.chat.id, f"Đã mở URL: {url}")
        # Đặt trạng thái chờ URL về False
        waiting_for_url[message.chat.id] = False
    except Exception as e:
        bot.send_message(message.chat.id, f"Có lỗi xảy ra khi mở URL: {e}")


@bot.message_handler(
    func=lambda message: waiting_for_text.get(message.chat.id) == "Inbox")
...
@bot.message_handler(
    func=lambda
    message: waiting_for_program_name.get(message.chat.id) == "KillApp")
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            if program_name.lower() in proc.info['name'].lower():
                ...
                bot.send_message(message.chat.id, f"Đã kết thúc chương trình: {
                                 proc.info['name']}")
                break
        else:
            bot.send_message(
                message.chat.id,
                f"Không tìm thấy chương trình có tên: {program_name}")
        waiting_for_program_name[message.chat.id] = None
    except Exception as e:
        bot.send_message(
            message.chat.id, f"Có lỗi xảy ra khi kết thúc chương trình: {e}")
